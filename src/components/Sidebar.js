    ...
    return (
        <div className='w-full h-full grid grid-cols-[64px,1fr] bg-white'>
            <div className='bg-slate-100 w-16 h-full rounded-tr-lg rounded-br-lg py-5 text-slate-600 flex flex-col justify-between'>
                <div>
                    <NavLink className={({ isActive }) => `w-16 h-16 flex justify-center items-center cursor-pointer hover:bg-slate-200 rounded ${isActive && "bg-slate-200"}`} title='chat'>
                ...
                <div className='h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar'>
...

                    {
                        allUser.map((conv, index) => {
                            return (
                                <NavLink to={"/" + conv?.userDetails?._id} key={conv?._id} className='flex items-center gap-4 py-3 px-4 border border-transparent hover:border-primary rounded hover:bg-slate-100 cursor-pointer'>
                                    <div>
                                        <Avatar
